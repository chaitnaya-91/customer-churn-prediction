...
def train_rideshare_models(csv_file_stream, tune_xgboost=False):
    """
    Trains SVM, Random Forest, and XGBoost on the ride-share churn dataset.
    Features: age, gender, city, membership_type, tenure_months, total_rides,
              average_ride_cost, days_since_last_ride, cancellation_rate,
              customer_satisfaction_score, payment_method, app_usage_hours_per_week
    ...
    cat_cols = ['gender', 'city', 'membership_type', 'payment_method']
    for col in cat_cols:
    ...
    # Extract features
    expected_cols = [
        'age', 'gender', 'city', 'membership_type', 'tenure_months', 'total_rides',
        'average_ride_cost', 'days_since_last_ride', 'cancellation_rate',
        'customer_satisfaction_score', 'payment_method', 'app_usage_hours_per_week',
        'churn_status'
    ]
    ...
    # Encoding
    cat_encode = [c for c in ['gender', 'city', 'membership_type', 'payment_method'] if c in ds.columns]
    if cat_encode:
    ...
    joblib.dump(best_model, os.path.join('dump models', 'rideshare_best_model.pkl'))
    joblib.dump(scaler, os.path.join('dump models', 'rideshare_scaler.pkl'))
    joblib.dump(list(X.columns), os.path.join('dump models', 'rideshare_features.pkl'))
