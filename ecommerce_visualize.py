...
def generate_visualizations(csv_file_stream):
    ...
    plot_cols = [c for c in ['age', 'total_orders', 'average_order_value', 'customer_satisfaction_score'] if c in df.columns]
    if plot_cols:
        fig, axes = plt.subplots(1, len(plot_cols), figsize=(15, 5))
        if len(plot_cols) == 1: axes = [axes]
        for idx, col in enumerate(plot_cols):
            sns.histplot(df[col].dropna(), kde=True, ax=axes[idx], color='darkorchid')
    ...
    if not num_df.empty and len(num_df.columns) > 1:
        ...
        sns.heatmap(num_df.corr(), annot=True, cmap='magma', fmt=".2f", ax=ax)
    ...
    if missing.sum() > 0:
        ...
        sns.barplot(x=missing.index, y=missing.values, ax=ax, color='mediumorchid')
    ...
    # 5. Outlier Analysis
    outlier_cols = [c for c in ['age', 'average_order_value', 'days_since_last_order', 'return_rate'] if c in df.columns]
    if outlier_cols:
        fig, axes = plt.subplots(1, len(outlier_cols), figsize=(15, 5))
        if len(outlier_cols) == 1: axes = [axes]
        for idx, col in enumerate(outlier_cols):
            sns.boxplot(y=df[col].dropna(), ax=axes[idx], color='plum')
            axes[idx].set_title(f'Outliers in {col}')
        plt.tight_layout()
        visualizations['Outlier Analysis'] = {'type': 'image', 'content': _fig_to_base64(fig)}

    # 6. Categorical Analysis
    cat_cols = [c for c in ['churn_status', 'gender', 'city'] if c in df.columns]
    if cat_cols:
        fig, axes = plt.subplots(1, len(cat_cols), figsize=(18, 5))
        if len(cat_cols) == 1: axes = [axes]
        for idx, col in enumerate(cat_cols):
            sns.countplot(x=df[col].dropna(), ax=axes[idx], hue=df[col].dropna(), palette='Purples', legend=False)
    ...
    if 'tenure_months' in df.columns and 'churn_status' in df.columns:
        ...
        if not mapped.isnull().all():
            ...
            churn_by_tenure = temp.groupby('tenure_months')['churn_status'].mean().reset_index()
            fig, ax = plt.subplots(figsize=(12, 5))
            sns.lineplot(data=churn_by_tenure, x='tenure_months', y='churn_status', ax=ax, color='purple', marker='o')
